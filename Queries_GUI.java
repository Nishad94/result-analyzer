...
public class Queries_GUI {
	...
	List<String[]> filterByCriteria(String college, String branch,
		...
		if (college != "College") {
			...
			List<DBObject> db_list = q.filter_by_college(college,
					db_table.original_table).toArray();
		...
		if (branch != "Branch") {
			List<DBObject> db_list = q.filter_by_branch(branch,
					db_table.filtered_collection).toArray();
		...
		if (batch != "Batch") {
			...
			List<DBObject> db_list = q.filter_by_batch(batch,
					db_table.filtered_collection).toArray();
			System.out.println(db_cursor.count() + " ---  ");
			db_table.filtered_collection.drop();
			System.out.println(db_list.size());
		...
		if (year != "Year") {
			...
			System.out.println("Before year filter : "
					+ Long.toString(db_table.filtered_collection.count()));
			List<DBObject> db_list = q.filter_by_year(year,
					db_table.filtered_collection).toArray();
			System.out.println(db_cursor.count() + " ---  ");
			db_table.filtered_collection.drop();
			System.out.println(db_list.size());
